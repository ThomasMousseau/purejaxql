...
def make_train(config):
    ...
    singularclip_k = int(config.get("K", 0))
    singularclip_c = float(config.get("c", 1.0))

    if singularclip_k < 0:
        raise ValueError("K must be nonnegative.")
    if singularclip_k > 0 and singularclip_c < 1.0:
        raise ValueError("c must be at least 1.0 when singularclip is enabled.")
    ...
    def singularclip_weight(weight):
        original_shape = weight.shape
        matrix = (
            weight.reshape((-1, original_shape[-1])) if weight.ndim > 2 else weight
        )
        svd_dtype = jnp.promote_types(matrix.dtype, jnp.float32)
        u, singular_values, vh = jnp.linalg.svd(
            matrix.astype(svd_dtype), full_matrices=False
        )
        singular_values = jnp.clip(
            singular_values, 1.0 / singularclip_c, singularclip_c
        )
        clipped_matrix = (u * singular_values) @ vh
        return clipped_matrix.reshape(original_shape).astype(weight.dtype)

    def singularclip_params(params):
        flat_params = flatten_dict(params)
        clipped_flat_params = {
            key: singularclip_weight(value)
            if key[-1] == "kernel" and value.ndim >= 2
            else value
            for key, value in flat_params.items()
        }
        clipped_params = unflatten_dict(clipped_flat_params)
        if isinstance(params, FrozenDict):
            return freeze(clipped_params)
                ...
                def _learn_phase(carry, minibatch_and_target):
                    ...
                    if singularclip_k > 0:
                        train_state = jax.lax.cond(
                            train_state.grad_steps % singularclip_k == 0,
                            lambda state: state.replace(
                                params=singularclip_params(state.params)
                            ),
                            lambda state: state,
                            train_state,
                        )
